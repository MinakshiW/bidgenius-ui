...
const AuctionDetailsPage = () => {
  ...
  const checkBidderExistence = (owner_id) => {
    return axios.get(`http://127.0.0.1:8000/bidders/?owner=${owner_id}`)
      .then((response) => {
        if (response.status === 200 && response.data.length > 0) {
          return true; // Bidder exists
        } else {
          return false; // Bidder doesn't exist
        }
      })
      .catch((error) => {
        console.error("Error checking bidder existence:", error);
        return false;
      });
  };

  const registerBidder = (auction_id, bidType, owner_id) => {
    axios.post('http://127.0.0.1:8000/bidders/', {
      ...
      .then((response) => {
        console.log('Bidder registered:', response.data);
      })
      .catch((error) => {
  ...
  const handleBid = async (auction_id, bidType, owner_id) => {
    const bidderExists = await checkBidderExistence(owner_id);

    if (!bidderExists) {
      // If bidder does not exist, register the owner
      registerBidder(auction_id, bidType, owner_id);
    }

    // Redirect based on the bid type
    if (bidType === 'manual') {
      navigate(`/manualbid/${auction_id}`); // Redirect to manual bidding page
    } else if (bidType === 'automatic') {
      console.log("Automatic bidding started for auction ID:", auction_id);
    }
  };

  const handleManualBidding = (auction_id, owner_id) => {
    handleBid(auction_id, 'manual', owner_id);
  };

  const handleAutomaticBidding = (auction_id, owner_id) => {
    handleBid(auction_id, 'automatic', owner_id);
  };
